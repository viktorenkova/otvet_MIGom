...
Этот чек-лист описывает эксплуатационные проверки. С 2026-09-04 конфигурационный preflight и ручная staging-приёмка сами по себе не разрешают выпуск: дополнительно нужны независимый gate 500+100 на текущем bundle, минимум 1000 новых real-shadow событий и последовательный rollout. Актуальный протокол — `docs/independent-acceptance-implementation-2026-09-04.md`.
...
- `configuration_ready: true` и `failures: 0` в preflight; его `production_ready` остаётся false, пока отдельная полная release-приёмка не реализована и не пройдена;
- весь набор тестов проходит.

Preflight блокирует открытый CORS, debug-режим, незаданную версию, shadow/legacy knowledge mode, неготовую конфигурацию доставки и небезопасную конфигурацию включённых LLM/status-интеграций. Относительный путь SQLite, выключенные персональные статусы и короткий/отсутствующий operations token отмечаются предупреждениями. `--strict` не может стать зелёным только за счёт конфигурации. Наличие SMTP-настроек не подтверждает работу отправителя очереди или доставку в ящик.
