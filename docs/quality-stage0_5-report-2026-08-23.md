...
До этапа 1 доменный эксперт должен подтвердить или исправить 110 записей в `reports/routing-label-adjudication-110.json`. Без этого нормализация БЗ будет оптимизироваться под потенциально неверные expected scenario IDs. После подтверждения меток следующий инженерный этап — нормализация БЗ и таксономии сценариев.
