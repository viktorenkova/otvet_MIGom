...
OBJECT_PATTERNS = {
    ...
    "support": r"\b(?:поддержк\w*|оператор\w*|менеджер\w*|сотрудник\w*|обратн\w*\s+связ\w*|обратн\w*\s+звон\w*)\b",
...
OPERATION_PATTERNS = {
    ...
    "contact": r"\b(?:позвон\w*|перезвон\w*|связ\w*|контакт\w*|набр\w*|обратн\w*\s+звон\w*)\b",
...
def match_scenario(message: str, role: str) -> ScenarioDecision:
    ...
    generic_lot_question = bool(
        ...
        and re.search(r"\b(?:лот\w*|авто\w*|машин\w*)\b", text)
        and (
            re.fullmatch(
            ...
            or re.search(
                r"\b(?:вопрос\w*\s+(?:пока\s+)?общ\w*|не\s+знаю.{0,24}сформулир\w*|"
                r"что.{0,20}нужно.{0,20}(?:знать|сообщить)|какие.{0,16}данн\w*.{0,16}(?:нужн\w*|дать)|"
                r"сначала.{0,20}понять.{0,20}(?:данн\w*|информац\w*)|надо.{0,12}уточнен\w*)\b",
                text,
            )
        )
    )
