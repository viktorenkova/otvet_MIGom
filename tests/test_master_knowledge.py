...
def test_master_is_complete_and_human_readable_sections_are_in_sync() -> None:
    ...
    assert result["metrics"]["scenario_count"] == 141
    assert result["metrics"]["fact_count"] == 574
    assert result["metrics"]["knowledge_gap_count"] == 3
    assert result["metrics"]["expert_review_candidate_count"] == 5
...
def test_master_exposes_known_content_and_provenance_gaps() -> None:
    ...
    assert len(review_queue["records"]) == 5
    assert "Просроченных повторных проверок на 2026-08-24: 5" in text
